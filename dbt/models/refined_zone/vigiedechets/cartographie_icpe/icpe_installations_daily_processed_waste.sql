...
with installations_data as (
    select
        ...
        max(i.commune)
            as commune,
        max(i.code_commune_insee)
            as code_commune_insee,
        max(i.code_departement_insee)
            as code_departement_insee,
        max(i.code_region_insee)
            as code_region_insee,
        max(i.latitude)
            as latitude,
        max(i.longitude)
            as longitude,
        sum(i.quantite_autorisee)
...
left join {{ ref('isdnd_capacites_limites_50pct') }} as c
