...
with source as (
    select *
    from {{ source('trackdechets_production', 'registry_outgoing_texs') }} as b
    {% if is_incremental() %}
        where
            b."updatedAt"
...
select
    ...
    toNullable(
        toString("reportForPickupSiteName")
    ) as report_for_pickup_site_name,
    toNullable(
        toString("reportForPickupSiteAddress")
    ) as report_for_pickup_site_address,
    toNullable(
        toString("reportForPickupSitePostalCode")
    ) as report_for_pickup_site_postal_code,
    toNullable(
        toString("reportForPickupSiteCity")
    ) as report_for_pickup_site_city,
    toNullable(
        toString("reportForPickupSiteCountryCode")
    ) as report_for_pickup_site_country_code,
    assumeNotNull(
    ...
    assumeNotNull(
        toTimezone(toDateTime64("dispatchDate", 6), 'Europe/Paris')
    ) as dispatch_date,
    toNullable(
        toString("wasteDap")
    ) as waste_dap,
    ...
    assumeNotNull(
        splitByChar(
            ...
            cOALESCE(
                substring(
                    toString("parcelInseeCodes"),
                    2,
                    length("parcelInseeCodes") - 2
                ),
                ''
            )
        )
    ) as parcel_insee_codes,
    assumeNotNull(
        splitByChar(
            ',',
            cOALESCE(
                substring(
                    toString("parcelNumbers"), 2, length("parcelNumbers") - 2
                ),
                ''
            )
        )
    ...
    assumeNotNull(
        splitByChar(
            ...
            cOALESCE(
                substring(
                    toString("parcelCoordinates"),
                    2,
                    length("parcelCoordinates") - 2
                ),
                ''
            )
        )
    ) as parcel_coordinates,
    toNullable(
        toString("sisIdentifier")
    ) as sis_identifier,
    assumeNotNull(
        toString("destinationCompanyType")
    ) as destination_company_type,
    toNullable(
        toString("destinationCompanyOrgId")
    ) as destination_company_org_id,
    toNullable(
    ...
    toNullable(
        toString("destinationCompanyCity")
    ) as destination_company_city,
    toNullable(
        toString("destinationCompanyPostalCode")
    ) as destination_company_postal_code,
    toNullable(
        toString("destinationCompanyCountryCode")
    ) as destination_company_country_code,
    toNullable(
        toString("destinationDropSiteAddress")
    ) as destination_drop_site_address,
    toNullable(
        toString("destinationDropSitePostalCode")
    ) as destination_drop_site_postal_code,
    toNullable(
        toString("destinationDropSiteCity")
    ) as destination_drop_site_city,
    toNullable(
        toString("destinationDropSiteCountryCode")
    ) as destination_drop_site_country_code,
    toLowCardinality(
    ...
    assumeNotNull(
        splitByChar(
            ...
            cOALESCE(
                substring(
                    toString("destinationParcelInseeCodes"),
                    2,
                    length("destinationParcelInseeCodes") - 2
                ),
                ''
            )
        )
    ) as destination_parcel_insee_codes,
    assumeNotNull(
        splitByChar(
            ',',
            cOALESCE(
                substring(
                    toString("destinationParcelNumbers"),
                    2,
                    length("destinationParcelNumbers") - 2
                ),
                ''
            )
        )
    ) as destination_parcel_numbers,
    assumeNotNull(
        splitByChar(
            ',',
            cOALESCE(
                substring(
                    toString("destinationParcelCoordinates"),
                    2,
                    length("destinationParcelCoordinates") - 2
                ),
                ''
            )
        )
    ) as destination_parcel_coordinates,
    toNullable(
