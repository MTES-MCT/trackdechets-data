...
with source as (
    select *
    from {{ source('trackdechets_production', 'registry_transported') }} as b
    {% if is_incremental() %}
        where
            b."updatedAt"
...
select
    ...
    toLowCardinality(
        assumeNotNull(toString("reportForCompanyPostalCode"))
    ) as report_for_company_postal_code,
    toLowCardinality(
        assumeNotNull(toString("reportForTransportMode"))
    ) as report_for_transport_mode,
    assumeNotNull(
        toBool("reportForTransportIsWaste")
    ) as report_for_transport_is_waste,
    toNullable(
        toBool("reportForRecepisseIsExempted")
    ) as report_for_recepisse_is_exempted,
    toNullable(
        toString("reportForRecepisseNumber")
    ) as report_for_recepisse_number,
    toNullable(
        toString("reportForTransportAdr")
    ) as report_for_transport_adr,
    toNullable(
        toString("reportForTransportOtherTmdCode")
    ) as report_for_transport_other_tmd_code,
    assumeNotNull(
        splitByChar(
            ',',
            coalesce(
                substring(
                    toString("reportForTransportPlates"),
                    2,
                    length("reportForTransportPlates") - 2
                ),
                ''
            )
        )
    ) as report_for_transport_plates,
    toNullable(
    ...
    assumeNotNull(
        toTimezone(toDateTime64("collectionDate", 6), 'Europe/Paris')
    ) as collection_date,
    assumeNotNull(
        toTimezone(toDateTime64("unloadingDate", 6), 'Europe/Paris')
    ) as unloading_date,
    assumeNotNull(
