...
with source as (
    select * from {{ source('trackdechets_production', 'bspaoh') }} as b
    {% if is_incremental() %}
        where
            b."updatedAt"
...
select
    ...
    toLowCardinality(
        assumeNotNull(toString("wasteType"))
    ) as waste_type,
    toNullable(
        toString("wastePackagings")
    ) as waste_packagings,
    toNullable(
    ...
    toNullable(
        toInt256("emitterWasteQuantityValue")
    ) as emitter_waste_quantity_value,
    toNullable(
        toFloat64("emitterWasteWeightValue")
    ) as emitter_waste_weight_value,
    toNullable(
    ...
    toNullable(
        toTimezone(
            toDateTime64("handedOverToDestinationSignatureDate", 6),
            'Europe/Paris'
        )
    ) as handed_over_to_destination_signature_date,
    toNullable(
        toString("handedOverToDestinationSignatureAuthor")
    ) as handed_over_to_destination_signature_author,
    toNullable(
        toInt256("destinationReceptionWasteQuantityValue")
    ) as destination_reception_waste_quantity_value,
    toNullable(
    ...
    toNullable(
        toString("destinationReceptionWastePackagingsAcceptation")
    ) as destination_reception_waste_packagings_acceptation,
    toNullable(
    ...
    toLowCardinality(
        toNullable(toString("destinationOperationCode"))
    ...
    toNullable(
        toFloat64("destinationReceptionWasteAcceptedWeightValue")
    ) as destination_reception_waste_accepted_weight_value,
    toNullable(
        toFloat64("destinationReceptionWasteReceivedWeightValue")
    ) as destination_reception_waste_received_weight_value,
    toNullable(
        toFloat64("destinationReceptionWasteRefusedWeightValue")
    ) as destination_reception_waste_refused_weight_value,
    toNullable(
        toString("isDuplicateOf")
    ) as is_duplicate_of
from source
