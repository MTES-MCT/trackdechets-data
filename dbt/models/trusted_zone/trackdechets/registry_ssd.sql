...
with source as (
    select * from {{ source('trackdechets_production', 'registry_ssd') }} as b
    {% if is_incremental() %}
        where
            b."updatedAt"
...
select
    ...
    assumeNotNull(
        splitByChar(
            ...
            cOALESCE(
                substring(
                    toString("secondaryWasteCodes"),
                    2,
                    length("secondaryWasteCodes") - 2
                ),
                ''
            )
        )
    ) as secondary_waste_codes,
    assumeNotNull(
        splitByChar(
            ',',
            cOALESCE(
                substring(
                    toString("secondaryWasteDescriptions"),
                    2,
                    length("secondaryWasteDescriptions") - 2
                ),
                ''
            )
        )
    ) as secondary_waste_descriptions,
    toNullable(
        toTimezone(toDateTime64("dispatchDate", 6), 'Europe/Paris')
    ) as dispatch_date,
    toNullable(
        toTimezone(toDateTime64("useDate", 6), 'Europe/Paris')
    ) as use_date,
    assumeNotNull(
        toTimezone(toDateTime64("processingDate", 6), 'Europe/Paris')
    ) as processing_date,
    toNullable(
        toTimezone(toDateTime64("processingEndDate", 6), 'Europe/Paris')
    ) as processing_end_date,
    toLowCardinality(
    ...
    assumeNotNull(
        toString("administrativeActReference")
    ) as administrative_act_reference,
    toNullable(
