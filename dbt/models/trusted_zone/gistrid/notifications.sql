...
renamed as (
    select
        ...
        toFloat64(toNullable({{ adapter.quote("quantité autorisée") }}))                      as quantite_autorisee,
        {{ adapter.quote("unité") }}                                          as unite,
        toInt256(toNullable({{ adapter.quote("Nombre total de transferts autorisés") }}))      as nombre_total_transferts_autorises,
